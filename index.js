...
function getMediaMessage(message, wantedType) {
    ...
    const quotedContent = m?.extendedTextMessage?.contextInfo?.quotedMessage;
    if (quotedContent) {
        const unwrapped = quotedContent?.viewOnceMessage?.message || quotedContent;
...
const COMMANDS = [
    ...
    },
    {
        ...
        handler: async ({ sock, message, args }) => {
            if (!args) {
                return reply(sock, message, `Contoh: *${PREFIX}say halo dunia*`);
            }
    ...
    },
    {
        ...
        handler: async ({ sock, message }) => {
            ...
            await sock.sendMessage(message.key.remoteJid, { image: png }, { quoted: message });
        },
    },
    {
        name: "stikergif",
        ...
        handler: async ({ sock, message }) => {
            ...
            const buffer = await downloadMedia(media.msg);
            const tempVideo = path.join(os.tmpdir(), `bowabot_video_${Date.now()}.mp4`);
            const tempWebp = path.join(os.tmpdir(), `bowabot_sticker_${Date.now()}.webp`);
...
const commandMap = Object.fromEntries(COMMANDS.map((c) => [c.name, c]));
...
async function startBot() {
    ...
    sock.ev.on("connection.update", ({ connection, lastDisconnect, qr }) => {
        ...
        if (connection === "close") {
            ...
            if (statusCode === DisconnectReason.loggedOut) {
                console.log("Sesi tidak valid, menghapus session dan meminta scan ulang...");
    ...
    sock.ev.on("messages.upsert", async ({ messages }) => {
        ...
        try {
            await cmd.handler({ sock, message, args });
        } catch (error) {
            console.error("Error saat menjalankan perintah:", error);
            await reply(sock, message, "Terjadi kesalahan saat menjalankan perintah. Coba lagi.");
        }
    });
}
